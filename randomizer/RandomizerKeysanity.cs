...
public class RandomizerKeysanity {
    ...
    private bool GetDoorHint(MoonGuid guid) {
        return 1 == (1 & (inventory.GetRandomizerItem(312) >> RandomizerLocationManager.KeystoneDoors[guid].Index));
    }

    private bool GetDoorHint(int id) {
        return GetDoorHint(reverseDoorKeyMap[id]);
    }

    private void SetDoorHint(MoonGuid guid) {
        inventory.SetRandomizerItem(312, inventory.GetRandomizerItem(312) | (1 << RandomizerLocationManager.KeystoneDoors[guid].Index));
    }
