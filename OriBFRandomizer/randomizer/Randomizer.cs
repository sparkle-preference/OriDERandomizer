...
public static class Randomizer
{
    ...
    public static void initialize()
    {
        try
        {
            ...
            Randomizer.StringKeyPickupTypes = new List<string>
                {"TP", "SH", "NO", "WT", "MU", "HN", "WP", "RP", "WS", "TW", "NB"};
            ...
            Console.WriteLine(typeof(RandomizerMessageProvider).AssemblyQualifiedName);
            Randomizer.MessageProvider =
               ScriptableObject.CreateInstance<RandomizerMessageProvider>();
            if(RandomizerUI.Instance)
            ...
            Randomizer.HotColdTypes = new HashSet<string>()
                {"EV", "RB17", "RB19", "RB21", "RB28", "SK", "TPForlorn", "TPHoru", "TPGinso", "TPValley", "TPSorrow"};
    ...
    public static void InitializeOnce()
    {
        ...
        RandomizerSettings.InitializeSettings();
    ...
    {
        return Sein.World.Events.WindRestored && Scenes.Manager.CurrentScene != null &&
               Scenes.Manager.CurrentScene.Scene != "forlornRuinsResurrection" &&
               Scenes.Manager.CurrentScene.Scene != "forlornRuinsRotatingLaserFlipped";
    }
...
//            InitializeOnce();
        ...
        if (Characters.Sein && SkillTreeManager.Instance != null &&
            SkillTreeManager.Instance.NavigationManager.IsVisible)
        {
            ...
            {
                SkillTreeManager.Instance.NavigationManager.FadeAnimator.SetParentOpacity(RandomizerSettings.QOL
                    .AbilityMenuOpacity);
            }
        }
        ...
        if (RandomizerRebinding.ReturnToStart.IsPressed() && Characters.Sein && !SafeIsBashing &&
    ...
    public static void showProgress()
    {
        try
        {
            ...
            else if (Randomizer.Shards)
            {
                ...
                text +=
                    $"*WV{(Keys.GinsoTree ? ": Found*" : "*: ????")}  #GS{(Keys.ForlornRuins ? ": Found#" : "#: ????")}  @SS{(Keys.MountHoru ? ": Found@" : "@: ????")}";
    ...
    public static void changeColor()
    {
        if (Characters.Sein)
        {
            Characters.Sein.PlatformBehaviour.Visuals.SpriteRenderer.material.color = new Color(FixedRandom.Values[0],
                FixedRandom.Values[1], FixedRandom.Values[2], 0.5f);
        }
    }
    ...
    public static void UpdateMessages()
    {
        if (Randomizer.MessageQueueTime <= 0)
        {
            ...
            if (message != "")
            {
                ...
                Game.UI.Hints.Show(Randomizer.MessageProvider, (HintLayer) 5,
                    (float) Randomizer.MessageQueueTime / 30f + 1f);
            }
        }
    ...
    public static void EnterDoor(Vector3 position)
    {
        ...
        int num =
            (int) (Math.Floor((double) ((int) position.x) / Randomizer.GridFactor) * Randomizer.GridFactor) * 10000 +
            (int) (Math.Floor((double) ((int) position.y) / Randomizer.GridFactor) * Randomizer.GridFactor);
            ...
            for (int j = -1; j <= 1; j++)
            {
                if (Randomizer.DoorTable.ContainsKey(num + (int) Randomizer.GridFactor * (10000 * i + j)))
                {
                    Characters.Sein.Position =
                        (Vector3) Randomizer.DoorTable[num + (int) Randomizer.GridFactor * (10000 * i + j)];
                    return;
                }
            }
        }
        ...
        for (int k = -2; k <= 2; k += 4)
        {
            for (int l = -1; l <= 1; l++)
            {
                ...
                {
                    Characters.Sein.Position =
                        (Vector3) Randomizer.DoorTable[num + (int) Randomizer.GridFactor * (10000 * k + l)];
                    return;
                }
            }
        }

        Randomizer.printInfo("Error using door at " + ((int) position.x).ToString() + ", " +
    ...
    public static void Print(string text, int seconds, bool mute, bool setMessage, bool devOnly, bool immediate)
    {
        ...
        if (immediate)
        {
            ...
            Game.UI.Hints.Show(Randomizer.MessageProvider, (HintLayer) 5, (float) seconds);
            if (setMessage)
            {
                Message = text;
            ...
            if (Randomizer.LastTick < old_tick)
            {
                ...
                if (Characters.Sein)
                {
                    if (!Characters.Sein.IsSuspended && Scenes.Manager.CurrentScene != null)
                    {
                        ...
                        if (Characters.Sein.Position.y > 935f && Randomizer.Inventory.FinishedGinsoEscape &&
                        ...
                        if (Characters.Sein.Position.y > -225f &&
                            Scenes.Manager.CurrentScene.Scene == "forlornRuinsResurrection")
                        {
                            if (SafeIsBashing)
                                Characters.Sein.Abilities.Bash.BashGameComplete(0f);
                    ...
                    if (RandomizerSyncManager.NetworkFree)
                    {
                        foreach (GameMapTeleporter gameMapTP in TeleporterController.Instance.Teleporters)
                        {
                            if (gameMapTP.Activated)
                                continue;
                            if (gameMapTP.Identifier == "ginsoTree" && get(1024) == 1 &&
                                RandomizerBonus.WaterVeinShards() >= 2)
                            {
                                TeleporterControllerExt.Activate(Randomizer.TeleportTable["Ginso"].ToString(), false);
                                if (RandomizerSettings.Customization.MultiplePickupMessages)
                                {
                            ...
                            else if (gameMapTP.Identifier == "forlorn" && get(1025) == 1 &&
                                     RandomizerBonus.GumonSealShards() >= 2)
                            {
                                TeleporterControllerExt.Activate(Randomizer.TeleportTable["Forlorn"].ToString(), false);
                                if (RandomizerSettings.Customization.MultiplePickupMessages)
                                {
                            ...
                            else if (gameMapTP.Identifier == "mountHoru" && get(1026) == 1 &&
                                     RandomizerBonus.SunstoneShards() >= 2)
                            {
                                TeleporterControllerExt.Activate(Randomizer.TeleportTable["Horu"].ToString(), false);
                                if (RandomizerSettings.Customization.MultiplePickupMessages)
                                {
    ...
    public static bool ParseFlags(string seed, string[] rawFlags)
    {
        ...
        foreach (string rawFlag in rawFlags)
        {
            ...
            {
                SeedMeta =
                    $"{String.Join(",", rawFlags.Select(f => f.ToLower().StartsWith("sync") ? "Sync" + cct(f.Skip(f.IndexOf('.') - 1)) : f).ToArray())}|{cct(seed.Skip(1).SkipWhile(c => Char.IsLower(c)))}"; // yeah that's right we're LINQ wizards baby anyways this is just a bit of censorship to make things a bit harder for people trying to cheat
            }
            ...
            if (flag.StartsWith("hotcold="))
            {
                ...
                Randomizer.HotColdTypes =
                    new HashSet<string>(rawFlag.Substring(8).Split(new char[] {'+'}).ToList<string>());
            }

            if (flag.StartsWith("sense="))
            {
                Randomizer.HotColdTypes =
                    new HashSet<string>(rawFlag.Substring(6).Split(new char[] {'+'}).ToList<string>());
            }
    ...
    public static void GetSenseFromSeedLine(int coords, string code, string id, string area)
    {
        ...
        if (Randomizer.HotColdTypes.Contains(code) ||
            Randomizer.HotColdTypes.Any((string t) => (code + id).StartsWith(t)))
        {
            if (Math.Abs(coords) > 100)
            {
                if (!Randomizer.HotColdItems.ContainsKey(coords))
                {
                    Randomizer.HotColdItems.Add(coords,
                        new RandomizerHotColdItem(Randomizer.HashKeyToVector(coords), Randomizer.HotColdSaveId));
        ...
        else if (code == "MS")
        {
            if (Math.Abs(coords) > 100)
            {
                ...
                {
                    Randomizer.HotColdFrags.Add(coords,
                        new RandomizerHotColdItem(Randomizer.HashKeyToVector(coords), Randomizer.HotColdSaveId));
                    Randomizer.HotColdSaveId++;
                }
            }
    ...
    private static HashSet<int>
        knownUnknowns = new HashSet<int>()
            {-1, 2, -1640264}; // remove -1640264 once appropriate seedgen changes happen ig?
    ...
    public static void ApplyGrabForgiveness()
    {
        ...
        if (new Rect(152.26f, -298.6f, 0.02f, 0.7f).Contains(Characters.Sein.PlatformBehaviour.PlatformMovement
                .Position2D))
        {
    ...
    public static void SetupNewGame()
    {
        ...
        TeleporterControllerExt.RemoveCustomTeleporters();
